---
title: "获取当前设置的推荐规则列表"
fullPath: "/uAjLw4CM/ukTMukTMukTM/application-v6/app_recommend_rule/list"
updateTime: "1665563094000"
---
...
| HTTP Method | GET |
| 支持的应用类型 | custom |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `application:app_recommend_rule:readonly` 获取应用的推荐规则信息 |
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `page_size` | `int` | 是 | 分页大小<br>**示例值**：10<br>**数据校验规则**： - 最大值：`50` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `id` | `string` | 推荐规则 ID |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `name` | `string` | 推荐规则名称 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `status` | `string` | 推荐规则启用状态<br>**可选值有**：<br>- `open`: 启用 - `closed`: 停用 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `item_type` | `string` | 推荐应用项类型<br>**可选值有**：<br>- `application`: 应用类型 - `link`: 链接类型 |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `name` | `string` | 推荐应用项名称 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `default_locale` | `string` | 链接类型应用项的默认展示语种（应用类型该字段为空）<br>**可选值有**：<br>- `zh_cn`: 简体中文 - `zh_hk`: 繁体中文（中国香港） - `zh_tw`: 繁体中文（中国台湾） - `en_us`: 英文 - `ja_jp`: 日文 |
