...
> **Tip**: - 此接口以Server-sent Events（SSE）方式更新问答结果，需要注意响应体格式
...
| 支持的应用类型 | custom |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `aily:knowledge:ask` 执行智能伙伴数据知识问答 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| `app_id` | `string` | 飞书 Aily 平台的AppID，通过飞书 Aily 平台创建应用获取<br>**示例值**："spring_5862e4fea8__c"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `255` 字符 |
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `message` | `aily_knowledge_message` | 是 | 输入消息（message包含content参数，当前仅支持纯文本输入） |
| &nbsp;&nbsp;└ `content` | `string` | 否 | 消息内容<br>**示例值**："推荐一部电影"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `65535` 字符 |
| `data_asset_ids` | `string\[\]` | 否 | 控制知识问答所依据的数据知识范围，默认值为空，此时不限制数据知识范围<br>**示例值**：["asset_aadg2b5os5wjg"]<br>**数据校验规则**：<br>- 长度范围：`0` ～ `65535` |
| `data_asset_tag_ids` | `string\[\]` | 否 | 控制知识问答所依据的数据知识分类范围，默认值为空，此时不限制数据知识分类范围<br>**示例值**：["spring_5862e4fea8__c__asset_tag_aadg2b5ql4gbs"]<br>**数据校验规则**：<br>- 长度范围：`0` ～ `65535` |
...
| `msg` | `string` | 错误描述 |
| `data` | `\-` | \- |
| &nbsp;&nbsp;└ `status` | `string` | 响应状态，枚举值<br>**可选值有**：<br>- `processing`: 当前知识问答正在处理中 - `finished`: 当前知识问答处理完成 |
| &nbsp;&nbsp;└ `finish_type` | `string` | 结束类型，枚举值<br>**可选值有**：<br>- `qa`: 执行数据知识问答 - `faq`: 执行标准问答对 |
| &nbsp;&nbsp;└ `message` | `aily_knowledge_message` | 响应消息 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `content` | `string` | 消息内容 |
| &nbsp;&nbsp;└ `process_data` | `aily_knowledge_ask_process_data` | 知识问答运行过程结构化数据，status=finished 且 finish_type=qa 时返回 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `chart_dsls` | `string\[\]` | 有数据分析时，根据数据生成的图表描述，按markdown语义描述 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `chunks` | `string\[\]` | 召回的知识视图切片的文本数据 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `sql_data` | `string\[\]` | 有数据分析时，查询到数据结果，每个元素为 json 序列化后的数据结果 |
| &nbsp;&nbsp;└ `faq_result` | `aily_knowledge_faq` | 匹配标准问答对结果，status=finished 且 finish_type=faq时返回 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `question` | `string` | 匹配问题 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `answer` | `string` | 匹配描述 |
| &nbsp;&nbsp;└ `has_answer` | `boolean` | 是否有结果，true 则 代表 message 中的内容是通过配置知识而生成的 |
...
| 200 | 2700033 | failed to ask knowledges | 执行数据知识问答失败，请结合文档检查请求参数 |
| 200 | 2700034 | ask is forbidden, please contact developer | 数据知识问答被禁用，请联系客服处理 |
| 200 | 2700035 | run time too long | 数据知识问答执行超时，请重试 |
