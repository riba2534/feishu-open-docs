...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `hire:interviewer` 管理面试官信息 `hire:interviewer:readonly` 获取面试官信息 |
...
| `verify_status` | `int` | 否 | 认证状态<br>**示例值**：1<br>**可选值有**：<br>- `1`: 未认证 - `2`: 已认证 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `verify_status` | `int` | 认证状态<br>**可选值有**：<br>- `1`: 未认证 - `2`: 已认证 |
