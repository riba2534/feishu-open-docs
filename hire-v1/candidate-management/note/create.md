...
| `application_id` | `string` | 否 | 投递ID，可通过[获取投递列表](https://open.larkoffice.com/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/application/list)获取<br>**示例值**："6891565253964859661" |
...
| `privacy` | `int` | 否 | 备注私密属性（默认为公开）<br>**示例值**：1<br>**可选值有**：<br>- `1`: 私密 - `2`: 公开<br>**默认值**：`2` |
| `notify_mentioned_user` | `boolean` | 否 | 是否通知被@的用户<br>**示例值**：false<br>**默认值**：`false` |
| `mention_entity_list` | `mention_entity\[\]` | 否 | 被@用户列表 |
| &nbsp;&nbsp;└ `offset` | `int` | 是 | 被@用户在 content 中的偏移量 - 取值范围：0 ~ content.length<br>**示例值**：3 |
