---
title: "通过员工 ID 获取入职信息"
fullPath: "/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/employee/get"
updateTime: "1725885851000"
---
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `hire:employee:readonly` 获取招聘员工信息 `hire:employee` 更新招聘员工信息 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| `employee_id` | `string` | 员工ID，可通过[通过投递 ID 获取入职信息](https://open.larkoffice.com/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/employee/get_by_application)接口获取<br>**示例值**："7379910335417927975" |
...
| `department_id_type` | `string` | 否 | 指定查询结果中的部门 ID 类型。关于部门 ID 的详细介绍，可参见[部门资源介绍](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/field-overview)。<br>**示例值**："department_id"<br>**可选值有**：<br>- `open_department_id`: 由系统自动生成的部门 ID， ID前缀固定为 `od-`，在租户内全局唯一。 - `department_id`: 支持用户自定义配置的部门 ID。自定义配置时可复用已删除的 department_id，因此在未删除的部门范围内，department_id 具有唯一性。 - `people_admin_department_id`: 以 people_admin_department_id 来标识部门，该 ID 类型即将下线，不推荐使用<br>**默认值**：`people_admin_department_id` |
