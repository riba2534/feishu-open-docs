...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `page_size` | `int` | 否 | 分页大小<br>**示例值**：100<br>**默认值**：`100`<br>**数据校验规则**： - 最大值：`200` |
...
| `topic` | `string` | 是 | 需要查询的任务分组主题，如「待办」、「已办」等<br>**示例值**："1"<br>**可选值有**：<br>- `1`: 待办审批 - `2`: 已办审批 - `3`: 已发起审批 - `17`: 未读知会 - `18`: 已读知会 |
...
| `data` | `\-` | \- |
| &nbsp;&nbsp;└ `tasks` | `task\[\]` | 任务列表 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `topic` | `string` | 任务所属的任务分组，如「待办」、「已办」等<br>**可选值有**：<br>- `1`: 待办审批 - `2`: 已办审批 - `3`: 已发起审批 - `17`: 未读知会 - `18`: 已读知会 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `status` | `string` | 任务状态<br>**可选值有**：<br>- `1`: 待办 - `2`: 已办 - `17`: 未读 - `18`: 已读 - `33`: 处理中，标记完成用 - `34`: 撤回 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `process_status` | `string` | 流程实例状态<br>**可选值有**：<br>- `0`: 无流程状态，不展示对应标签 - `1`: 流程实例流转中 - `2`: 已通过 - `3`: 已拒绝 - `4`: 已撤销 - `5`: 已终止 |
