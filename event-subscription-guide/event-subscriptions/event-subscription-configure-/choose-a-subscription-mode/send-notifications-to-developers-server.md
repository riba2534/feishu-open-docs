...
- 
加密推送的事件示例如下，你的本地服务器接收到加密事件体后，需要进行解密处理。解密步骤参考[接收事件](https://open.larkoffice.com/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/encrypt-key-encryption-configuration-case)。
...
配置 Encrypt Key 后，你本地的服务器可以基于 Encrypt Key 进行签名校验，以保证应用接收到的都是飞书开放平台推送的合法事件。有关签名校验的详细介绍，请参考[接收事件](https://open.larkoffice.com/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/encrypt-key-encryption-configuration-case)。
...
4. 进入 **事件配置** 页签，在 **订阅方式** 区域点击编辑图标。
    ...
    ![](https://sf3-cn.feishucdn.com/obj/open-platform-opendoc/cbca2aafb65c6a174017c337a178ccf4_jexH1K35ZT.png?height=482&lazyload=true&maxWidth=600&width=1282)
...
5. 选择 **将事件发送至开发者服务器**，并输入请求地址后，点击 **保存**。
    ...
    ![](https://sf3-cn.feishucdn.com/obj/open-platform-opendoc/644dc927531fb883770160fec26b6d93_qq8qFiRCeF.png?height=874&lazyload=true&maxWidth=600&width=1696)
...
- **已配置 Encrypt Key**
  ...
  ```
	...
	应用需要先解密，然后从解密后的内容中提取出 `challenge` 值，并在 1 秒内原样返回该值作为响应。有关解密方法的详细介绍，请参考[接收事件](https://open.larkoffice.com/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/encrypt-key-encryption-configuration-case)。解密后的 POST 请求示例：
...
| 编程语言 | 示例代码与解析 |
| --- | --- |
| Java | [集成 Servlet 容器，将事件发送至开发者服务器](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/server-side-sdk/java-sdk-guide/handle-events#d9800e1) |
| Golang | [将事件发送至开发者服务器](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/server-side-sdk/golang-sdk-guide/handle-events#b8ccabc5) |
| Python | [将事件发送至开发者服务器](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/server-side-sdk/python--sdk/handle-events#b8ccabc5) |
| Node.js | [将事件发送至开发者服务器](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/server-side-sdk/nodejs-sdk/handling-events#b8ccabc5) |
