...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| `card_id` | `string` | 卡片实体 ID。通过[创建卡片实体](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/cardkit-v1/card/create)获取。<br>**示例值**："7355372766134157313"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `20` 字符 |
...
| &nbsp;&nbsp;└ `data` | `string` | 是 | 卡片 JSON 数据的内容。<br>**注意**： - 仅支持 JSON 2.0 版本的卡片结构。<br>- 以下示例值未转义，使用时请注意将其转为 JSON 序列化后的字符串。<br>**示例值**："{\"schema\":\"2.0\",\"header\":{\"title\":{\"content\":\"项目进度更新提醒\",\"tag\":\"plain_text\"}},\"body\":{\"elements\":[{\"tag\":\"markdown\",\"content\":\"截至今日，项目完成度已达80%\"}]}}"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `1000000` 字符 |
