...
获取文档、电子表格、多维表格等文件的历史访问记录，包括访问者的 ID、姓名、头像和最近访问时间。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=drive&version=v1&resource=file.view_record&method=list)
...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.feishu.cn/open-apis/drive/v1/files/:file_token/view_records</md-td>
    </md-tr>
    <md-tr>
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	文件 token。获取方式参考[文件概述](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file/file-overview)。
...
<md-enum-item key="wiki" >知识库文档</md-enum-item>
...
<md-dt-tr level="0">
	<md-dt-td>
	<md-text type="field-name" >viewer_id_type</md-text>
	</md-dt-td>
	<md-dt-td>
	<md-text type="field-type" >string</md-text>
	</md-dt-td>
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	<md-text type="field-type" >file.view_record\[\]</md-text>
	</md-dt-td>
	<md-dt-td>
	访问记录列表
	</md-dt-td>
...
<md-dt-tr level="2">
	<md-dt-td>
	<md-text type="field-name" >viewer_id</md-text>
	</md-dt-td>
	<md-dt-td>
	<md-text type="field-type" >string</md-text>
	</md-dt-td>
...
<md-dt-tr level="2">
	<md-dt-td>
	<md-text type="field-name" >last_view_time</md-text>
	</md-dt-td>
	<md-dt-td>
	<md-text type="field-type" >string</md-text>
	</md-dt-td>
...
<md-tr>
  ...
  <md-td>服务内部错误，包括服务端内部超时、错误码没处理等。</md-td>
</md-tr>
...
* 参数填写异常，如 page_size 小于等于 0</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>`file_token` 对应的文件在云空间中不存在。可能原因与解决方案如下所示：

- 文件不存在或 token 填写错误：请检查是否正确填写了 `file_token`。放置在云空间文件夹中的文件和知识库中的文件，其 token 获取方式不同。详情参考[文件概述-文件 token 获取方式](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file/file-overview)。
- 文件类型不匹配：请求中提供的 `file_type` 与 `file_token` 对应的实际文件的类型不匹配。请检查 `file_type` 的类型是否与文件的实际类型一致。</md-td>
</md-tr>
