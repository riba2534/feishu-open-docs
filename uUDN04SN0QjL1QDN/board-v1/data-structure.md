...
| theme_text_background_color_code | int | 主题色表盘下文字背景颜色 code 值 | code 对应主题下文字背景颜色色表盘上颜色的下标值，同一个 code 值，对于不同主题会有不同的颜色，可以做到文字背景颜色随主题切换功能，具体可参考 [主题简介](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/board-v1/theme-introduction)。与 text_background_color 作用相同，推荐使用此字段。同时设置时 text_background_color 生效 |
...
| 名称 | 数据类型 | 描述 | 属性 |
| --- | --- | --- | --- |
| token | string | 图片token，通过云文档下的素材上传接口 [上传素材](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/media/upload_all) 上传图片后返回的 token | required |
...
| &nbsp;&nbsp;└ `snap_to` | `string` | `连接图形的方向`<br>- `top`: 连接图形的上边 - `right`: 连接图形的右边 - `bottom`: 连接图形的下边 - `left`: 连接图形的左边 | `optional` |
...
| `shape` | `string` | `连线样式`<br>- `straight`: 直线 - `polyline`: 折线 - `curve`: 曲线 - `right_angled_polyline`: 直角折线 | `optional，默认为直线` |
