...
| 支持的应用类型 | custom,isv |
| 权限要求             订阅该事件所需的权限，开启其中任意一项权限即可订阅 开启任一权限即可 | `base:field:read` 获取字段信息 `bitable:app` 查看、评论、编辑和管理多维表格 `drive:drive` 查看、评论、编辑和管理云空间中所有文件 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `action` | `string` | 字段变更类型。枚举值有： - field_added：新增字段 - field_edited：修改字段 - field_deleted：删除字段 |
