...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `base:workflow:write` 更新自动化流程/工作流状态 `bitable:app` 查看、评论、编辑和管理多维表格 |
...
| `workflow_id` | `string` | 自动化工作流 ID，通过[列出自动化流程](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-workflow/list)接口获取。<br>**示例值**："730887xxxx552638996" |
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `status` | `string` | 是 | 自动化状态。可选值： - Enable：开启自动化流程 - Disable：关闭自动化流程<br>**示例值**："Enable" |
