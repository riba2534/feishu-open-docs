...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `base:form:update` 更新表单数据 |
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `form_name` | `string` | 是 | 升级后的表单名称<br>**示例值**："文档问题反馈"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `100` 字符 - 正则校验：`^[^[\]]+$` |
| `display_mode` | `string` | 是 | 表单布局模式。<br>**示例值**："one_question_per_page"<br>**可选值有**：<br>- `traditional`: 传统布局 - `one_question_per_page`: 一页一题布局 |
