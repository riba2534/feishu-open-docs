---
title: "获取表单元数据"
fullPath: "/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-form/get"
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `base:form:read` 获取表单数据 `bitable:app` 查看、评论、编辑和管理多维表格 `bitable:app:readonly` 查看、评论和导出多维表格 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `shared` | `boolean` | 是否开启表单分享，使表单支持填写。可选值： - true：支持填写 - false：不支持填写 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `submit_limit_once` | `boolean` | 是否将填写次数限制为一次。可选值： - true：设置表单仅支持填写一次 - false：不限制表单填写次数 |
