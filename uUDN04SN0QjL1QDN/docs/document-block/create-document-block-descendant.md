---
title: "创建嵌套块"
fullPath: "/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-descendant/create"
updateTime: "1752809928000"
---

# 创建嵌套块

在指定块的子块列表中，新创建一批有父子关系的子块，并放置到指定位置。如果操作成功，接口将返回新创建子块的富文本内容。调用该接口前，你可参考[文档概述-基本概念](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)了解块的父子关系规则。当创建的子块中含有 GridColumn、TableCell、Callout 时其中至少需要包含一个子块 ，即内容为空时也需要填入一个空 Text Block 作为子块。
...
| `block_id` | `string` | 父块的`block_id`，表示为其创建一批子块。如果需要对文档树根节点创建子块，可将 `document_id` 填入此处。你可调用[获取文档所有块](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list)获取文档中块的 block_id。了解块的父子关系规则，参考[文档概述-基本概念](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)<br>**示例值**："doxcnO6UW6wAw2qIcYf4hZabcef" |
...
| `descendants` | `block\[\]` | 是 | 添加的含有父子关系的嵌套块列表，包括父块和子块。了解块的父子关系规则，参考[文档概述-基本概念](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)<br>**数据校验规则**：<br>- 长度范围：`1` ～ `1000` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `content` | `string` | 是 | 文本内容。要实现文本内容的换行，你可以： - 在传入的文本内容中添加 `\n` 实现软换行（Soft Break，与在文档中通过操作 `Shift + Enter` 的效果一致） - 创建一个新的文本 Block，实现两个文本 Block 之间的硬换行（Hard Break，与在文档中通过操作 `Enter` 的效果一致）<br>**注意**：软换行在渲染时可能会被忽略，具体取决于渲染器如何处理；硬换行在渲染时始终会显示为一个新行。<br>**示例值**："文本" |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `view_type` | `int` | 否 | 类型<br>**示例值**：1<br>**可选值有**：<br>- `1`: 数据表 - `2`: 看板 |
| &nbsp;&nbsp;└ `callout` | `callout` | 否 | 高亮块 Block |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `background_color` | `int` | 否 | 高亮块背景色<br>**示例值**：1<br>**可选值有**：<br>- `1`: 浅红色 - `2`: 浅橙色 - `3`: 浅黄色 - `4`: 浅绿色 - `5`: 浅蓝色 - `6`: 浅紫色 - `7`: 中灰色 - `8`: 中红色 - `9`: 中橙色 - `10`: 中黄色 - `11`: 中绿色 - `12`: 中蓝色 - `13`: 中紫色 - `14`: 灰色 - `15`: 浅灰色 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `view_type` | `int` | 否 | 视图类型，卡片视图（默认）或预览视图<br>**示例值**：1<br>**可选值有**：<br>- `1`: 卡片视图 - `2`: 预览视图 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `iframe_type` | `int` | 否 | iframe 类型<br>**示例值**：1<br>**可选值有**：<br>- `1`: 哔哩哔哩 - `2`: 西瓜视频 - `3`: 优酷 - `4`: Airtable - `5`: 百度地图 - `6`: 高德地图 - `7`: Undefined - `8`: Figma - `9`: 墨刀 - `10`: Canva - `11`: CodePen - `12`: 飞书问卷 - `13`: 金数据 - `14`: Undefined - `15`: Undefined - `99`: Other |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `url` | `string` | 是 | iframe 目标 url（需要进行 url_encode）<br>**示例值**："https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1Hi4y1w7V7" |
| &nbsp;&nbsp;└ `image` | `image` | 否 | 图片 Block。了解如何在文档中插入图片，参考[文档常见问题-如何插入图片](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/faq)。 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `align` | `int` | 否 | 对齐方式<br>**示例值**：2<br>**可选值有**：<br>- `1`: 居左排版 - `2`: 居中排版 - `3`: 居右排版<br>**默认值**：`2` |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `caption` | `caption` | 否 | 图片描述 |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `content` | `string` | 否 | 描述的文本内容<br>**示例值**："caption" |
| &nbsp;&nbsp;└ `isv` | `isv` | 否 | 三方 Block |
| &nbsp;&nbsp;└ `add_ons` | `add_ons` | 否 | Add-ons |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `component_id` | `string` | 否 | 文档小组件 ID。该 ID 可通过调用[创建 BlockEntity](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/block-v2/entity/create) 接口，并从响应体中的 block_id 中获取，创建时使用的 `block_type_id` 需要与 `component_type_id` 一致。<br>**示例值**："7056882725002051603" |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `row_size` | `int` | 是 | 行数 - **创建块**接口中，该字段最大值为 9  - **创建嵌套块**接口中，在单个表格单元格不超过上限 2000 情况下，该字段无固定最大值<br>**示例值**：1<br>**数据校验规则**：<br>- 最小值：`1` |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `column_size` | `int` | 是 | 列数 - **创建块**接口中，该字段最大值为 9  - **创建嵌套块**接口中，该字段最大值为 100<br>**示例值**：1<br>**数据校验规则**：<br>- 最小值：`1` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `okr_id` | `string` | 否 | OKR ID，获取需要插入的 OKR ID 可见[获取用户的 OKR 列表](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/user-okr/list)<br>**示例值**："7076349900476448796" |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `align` | `int` | 否 | 对齐方式<br>**示例值**：2<br>**可选值有**：<br>- `1`: 居左排版 - `2`: 居中排版 - `3`: 居右排版<br>**默认值**：`1` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `url_type` | `string` | 是 | 链接类型<br>**示例值**："MessageLink"<br>**可选值有**：<br>- `MessageLink`: 消息链接 - `Undefined`: 未定义的链接类型 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `url` | `string` | 是 | 超链接指向的 url (需要 url_encode)<br>**示例值**："https%3A%2F%2Fopen.feishu.cn%2F" |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `comment_ids` | `string\[\]` | 否 | 评论 ID 列表。在创建 Block 时，不支持传入评论 ID；在更新文本 Block 的 Element 时，允许将对应版本已存在的评论 ID 移动到同一个 Block 内的任意 Element 中，但不支持传入新的评论 ID。如需查询评论内容请阅览「[获取回复](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file-comment-reply/list)」 API。<br>**示例值**：["1660030311959965796"] |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `align` | `int` | 否 | 对齐方式<br>**示例值**：1<br>**可选值有**：<br>- `1`: 居左排版 - `2`: 居中排版 - `3`: 居右排版 |
| &nbsp;&nbsp;└ `reference_synced` | `reference_synced` | 否 | 引用同步块，仅支持查询。获取引用同步块内容详见：[如何获取引用同步块的内容](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/faq#19b71234) |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `content` | `string` | 文本内容。要实现文本内容的换行，你可以： - 在传入的文本内容中添加 `\n` 实现软换行（Soft Break，与在文档中通过操作 `Shift + Enter` 的效果一致） - 创建一个新的文本 Block，实现两个文本 Block 之间的硬换行（Hard Break，与在文档中通过操作 `Enter` 的效果一致）<br>**注意**：软换行在渲染时可能会被忽略，具体取决于渲染器如何处理；硬换行在渲染时始终会显示为一个新行。 |
...
| 400 | 1770041 | open schema mismatch | 确认块父子关系是否合法。 文档场景中，可参考[文档概述-基本概念](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)了解块的父子关系规则。 |
