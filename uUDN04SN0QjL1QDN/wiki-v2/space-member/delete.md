...
> **Warning**: 使用tenant access token操作时，无法使用部门ID(opendepartmentid)删除知识空间成员。


> **Tip**: 知识空间具有[类型](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-overview)和[可见性](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-overview)的概念。不同的类型或可见性可以对本操作做出限制：
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `wiki:member:update` 更新知识空间成员 `wiki:wiki` 查看、编辑和管理知识库 |
