...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `wiki:node:copy` 创建知识空间节点副本 `wiki:wiki` 查看、编辑和管理知识库 |
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `target_parent_token` | `string` | 否 | 目标父节点 Token。 - 目标知识空间 ID 与目标父节点 Token 不可同时为空。<br>**示例值**："wikcnKQ1k3p******8Vabce"<br>**数据校验规则**：<br>- 长度范围：`0` ～ `999` 字符 |
| `target_space_id` | `string` | 否 | 目标知识空间 ID。 - 目标知识空间 ID 与目标父节点 Token 不可同时为空。<br>**示例值**："6946843325487912356" |
