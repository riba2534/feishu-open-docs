---
title: "创建知识空间节点"
fullPath: "/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-v2/space-node/create"
updateTime: "1749710137000"
---

# 创建知识空间节点

此接口用于在知识节点里创建[节点](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-overview)到指定位置。


> **Tip**: - 知识空间权限要求，当前使用的 access token 所代表的应用或用户拥有：
>   - **父节点**容器编辑权限
> 
> - 当前不支持创建`文件`类型节点。
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `wiki:node:create` 创建知识空间节点 `wiki:wiki` 查看、编辑和管理知识库 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| `space_id` | `string` | 知识空间id [获取方式](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-overview)<br>**示例值**："6704147935988285963" |
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `obj_type` | `string` | 是 | 文档类型，对于快捷方式，该字段是对应的实体的obj_type。<br>**示例值**："docx"<br>**可选值有**：<br>- `doc`: 已废弃，创建文档请使用`docx`。详情参考[旧版文档（Docs 1.0）创建能力下线说明](https://open.larkoffice.com/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/breaking-change/docs-create-ability-offline)。 - `sheet`: 表格 - `mindnote`: 思维导图 - `bitable`: 多维表格 - `file`: 文件 - `docx`: 新版文档 - `slides`: 幻灯片 |
| `parent_node_token` | `string` | 否 | 父节点 token。若当前节点为一级节点，父节点 token 为空。<br>**示例值**："wikcnKQ1k3p******8Vabcef" |
| `node_type` | `string` | 是 | 节点类型<br>**示例值**："origin"<br>**可选值有**：<br>- `origin`: 实体 - `shortcut`: 快捷方式 |
...
| 400 | 131010 | doc type is deprecated, please use docx. | 旧版文档创建能力已下线，详情参考[旧版文档（Docs 1.0）创建能力下线说明](https://open.larkoffice.com/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/breaking-change/docs-create-ability-offline)。 |
