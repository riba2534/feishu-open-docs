---
title: "获取知识空间列表"
fullPath: "/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-v2/space/list"
updateTime: "1755844515000"
---
...
- 使用 tenant access token 调用时，请确认应用或机器人拥有部分知识空间的访问权限，否则返回列表为空。参阅[如何将应用添加为知识库管理员（成员）](https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-v2/wiki-qa#b5da330b)。
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `wiki:space:retrieve` 查看知识空间列表 `wiki:wiki` 查看、编辑和管理知识库 `wiki:wiki:readonly` 查看知识库 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `visibility` | `string` | 表示知识空间可见性<br>**可选值有**：<br>- `public`: 公开空间，租户内所有用户可见，默认为成员权限。无法额外添加成员，但可以添加管理员 - `private`: 私有空间，仅对知识空间管理员、成员可见，需要手动添加管理员、成员 |
