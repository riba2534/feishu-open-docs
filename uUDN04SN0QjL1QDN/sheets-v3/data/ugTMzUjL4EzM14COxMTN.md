...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/:spreadsheetToken/values/:range </md-td>
    </md-tr>
    <md-tr>
...
- 可选值为 FormattedString，此时接口将计算并对日期、时间、或时间日期类型的数据格式化并返回格式化后的字符串，但不会对数字进行格式化。
    ...
    <md-tr>
      ...
      <md-td>否</md-td>
      <md-td>
       当单元格中包含@用户等涉及用户信息的元素时，该参数可指定返回的用户 ID 类型。默认为 `lark_id`，建议选择 `open_id` 或 `union_id`。了解更多，参考[用户身份概述](/ssl:ttdoc/home/user-identity-introduction/introduction)。可选值：
