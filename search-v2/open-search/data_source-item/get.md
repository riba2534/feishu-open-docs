...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `search:data_item:readonly` 查询数据项 `search:data_source` 查询、创建、修改和删除自定义搜索数据源、数据范式或数据项 `search:data_source:readonly` 查询自定义搜索数据源、数据范式或数据项 |
...
| `item_id` | `string` | 数据记录的唯一标识<br>**注意**：<br>- 该字段大小写敏感。 - 如果调用成功但返回结果为空数据，请检查该字段传值是否正确。<br>**示例值**："01010111" |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `structured_data` | `string` | 结构化数据（以 json 字符串传递），这些字段是搜索结果的展示字段(特殊字段无须在此另外指定);具体格式可参参考 [接入指南](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/search-v2/common-template-intergration-handbook) **请求创建数据项**部分。这里的示例遵循了”创建数据范式“部分中的数据范式示例，请按自己定义的数据范式填写数据 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `content` | `item_content` | 非结构化数据，如文档文本，飞书搜索会用来做召回 |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `format` | `string` | 内容的格式<br>**可选值有**：<br>- `html`: html格式 - `plaintext`: 纯文本格式 |
