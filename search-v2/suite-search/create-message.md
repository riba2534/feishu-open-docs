...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `search:message` 获取消息搜索结果 |
...
| `message_type` | `string` | 否 | 消息类型(file/image/media)<br>**示例值**："image"<br>**可选值有**：<br>- `file`: 文件 - `image`: 图片 - `media`: 视频 |
| `at_chatter_ids` | `string\[\]` | 否 | at用户user_id列表<br>**示例值**：["ou_7dab8a3d3cdcc9da365777c7ad535d62"] |
| `from_type` | `string` | 否 | 消息来自类型(bot/user)<br>**示例值**："user"<br>**可选值有**：<br>- `bot`: 机器人 - `user`: 用户 |
| `chat_type` | `string` | 否 | 会话类型(group_chat/p2p_chat)<br>**示例值**："group_chat"<br>**可选值有**：<br>- `group_chat`: 群聊 - `p2p_chat`: 单聊 |
