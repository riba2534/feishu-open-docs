...
| `type` | `int` | 否 | 用户组的类型。默认取值 `1`，表示普通用户组。<br>**示例值**：1<br>**可选值有**：<br>- `1`: 普通用户组 - `2`: 暂不支持使用该值 |
| `group_id` | `string` | 否 | 自定义用户组 ID。<br>**数据校验规则：**<br>- 最大长度：64 字符 - 校验规则：数字、大小写字母的组合，不能包含空格<br>**默认值**：空，由系统生成一个默认 ID。<br>**示例值**："g122817" |
...
| 403 | 42010 | not has all authority error | 应用没有全部员工权限。应用的通讯录权限范围必须为全部员工，否则会调用失败。了解应用通讯录权限范围，可参见[权限范围资源介绍](https://open.larkoffice.com/document/ukTMukTMukTM/uETNz4SM1MjLxUzM/v3/guides/scope_authority)。 |
