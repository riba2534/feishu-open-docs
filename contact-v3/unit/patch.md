...
调用该接口修改指定单位的名字。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=contact&version=v3&resource=unit&method=patch)
...
<md-tr>
  ...
  <md-td>单位名字和类型同时重复。在当前租户内，相同单位类型的单位不允许单位名字重复。你需要修改单位名字后重试。</md-td>
</md-tr>
