---
title: "获取部门直属用户列表"
fullPath: "/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/find_by_department"
updateTime: "1729664725000"
---
...
关于用户组织架构可见范围和通讯录权限范围的更多信息，可参见[权限范围资源介绍](https://open.larkoffice.com/document/ukTMukTMukTM/uETNz4SM1MjLxUzM/v3/guides/scope_authority)。
...
| `department_id` | `string` | 是 | 部门 ID，ID 类型与 department_id_type 的取值保持一致。<br>**说明**： - 根部门的部门 ID 为 0。 - 你可以调用[搜索部门](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/search)接口，通过部门名称关键词获取对应的部门 ID。<br>**示例值**：od-xxxxxxxxxxxxx |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `department_ids` | `string\[\]` | 用户所属部门的 ID 列表，一个用户可属于多个部门。ID 值的类型与查询参数中的 department_id_type 取值一致。<br>**字段权限要求（满足任一）**： `contact:user.department:readonly` 获取用户组织架构信息 `contact:contact:access_as_app` 以应用身份访问通讯录 `contact:contact:readonly` 读取通讯录 `contact:contact:readonly_as_app` 以应用身份读取通讯录 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `leader_user_id` | `string` | 用户的直接主管的用户ID。ID 值的类型与查询参数中的 user_id_type 取值一致。<br>**字段权限要求（满足任一）**： `contact:user.department:readonly` 获取用户组织架构信息 `contact:contact:access_as_app` 以应用身份访问通讯录 `contact:contact:readonly` 读取通讯录 `contact:contact:readonly_as_app` 以应用身份读取通讯录 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `department_path` | `department_detail\[\]` | 部门路径。<br>**注意**：使用应用身份（tenant_access_token）调用本接口时，响应结果中不会返回部门路径字段（department_path）。如需获取部门路径字段值，请为应用申请 **获取成员所在部门路径（contact:user.department_path:readonly）** API 权限，并使用用户身份（user_access_token）调用接口。<br>**字段权限要求**： `contact:user.department_path:readonly` 获取成员所在部门路径 |
