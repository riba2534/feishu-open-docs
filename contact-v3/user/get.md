...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `name` | `string` | 用户名称。<br>**注意**：用户名称分为默认名称和多语种版本的名称，该参数返回的是用户默认名称。<br>**字段权限要求（满足任一）**： `contact:user.base:readonly` 获取用户基本信息 `contact:contact:access_as_app` 以应用身份访问通讯录 `contact:contact:readonly` 读取通讯录 `contact:contact:readonly_as_app` 以应用身份读取通讯录 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `department_ids` | `string\[\]` | 用户所属部门的 ID 列表，一个用户可属于多个部门。ID 值的类型与查询参数 department_id_type 的取值保持一致。<br>**字段权限要求（满足任一）**： `contact:user.department:readonly` 获取用户组织架构信息 `contact:contact:access_as_app` 以应用身份访问通讯录 `contact:contact:readonly` 读取通讯录 `contact:contact:readonly_as_app` 以应用身份读取通讯录 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `leader_user_id` | `string` | 用户的直接主管的用户ID。ID 值的类型与查询参数 user_id_type 的取值保持一致。<br>**字段权限要求（满足任一）**： `contact:user.department:readonly` 获取用户组织架构信息 `contact:contact:access_as_app` 以应用身份访问通讯录 `contact:contact:readonly` 读取通讯录 `contact:contact:readonly_as_app` 以应用身份读取通讯录 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `department_path` | `department_detail\[\]` | 部门路径列表。<br>**注意**： - 该字段需要通过用户身份（user_access_token）调用接口才可以获取。 - 仅当[管理后台](https://feishu.cn/admin) > **组织架构** > **字段管理** > **展示字段** > **名片页** 内的 **部门** 设置为 **展示完整部门路径** 时，该字段才可以获取到完整部门路径。<br>**字段权限要求**： `contact:user.department_path:readonly` 获取成员所在部门路径 |
...
| 400 | 41012 | user id invalid error | 用户 ID 无效。你可以通过以下接口获取指定用户的用户 ID，然后使用正确的用户 ID 再次尝试调用接口。 - [搜索用户](https://open.larkoffice.com/document/ukTMukTMukTM/uMTM4UjLzEDO14yMxgTN) - [通过手机号或邮箱获取用户 ID](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/batch_get_id) |
