...
调用该接口获取通讯录中某一用户的信息，包括用户 ID、名称、邮箱、手机号、状态以及所属部门等信息。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=contact&version=v3&resource=user&method=get)
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	指定查询结果中的部门 ID 类型。关于部门 ID 的详细介绍，可参见[部门 ID 说明](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/field-overview#23857fe0)。
...
**注意**：用户名称分为默认名称和多语种版本的名称，该参数返回的是用户默认名称。
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	用户所属部门的 ID 列表，一个用户可属于多个部门。ID 值的类型与查询参数 department_id_type 的取值保持一致。
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	用户的直接主管的用户ID。ID 值的类型与查询参数 user_id_type 的取值保持一致。
...
<md-dt-tr level="3">
	...
	<md-dt-td>
	排序信息对应的部门 ID，表示用户所在的、且需要排序的部门。ID 类型与查询参数 department_id_type 的取值保持一致。
	</md-dt-td>
</md-dt-tr>
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	自定义字段。了解自定义字段可参见[自定义字段资源介绍](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/custom_attr/overview)。
...
<md-dt-tr level="4">
	...
	<md-dt-td>
	枚举类型中选项的选项值。当返回该值时，还会返回 option_id 参数，取值为枚举值的 ID。
	</md-dt-td>
</md-dt-tr>
...
**注意**：
- 该字段需要通过用户身份（user_access_token）调用接口才可以获取。
- 仅当[管理后台](https://feishu.cn/admin) > **组织架构** > **字段管理** > **展示字段** > **名片页** 内的 **部门** 设置为 **展示完整部门路径** 时，该字段才可以获取到完整部门路径。
...
<md-tr>
  ...
  <md-td>参数错误。你需要检查输入参数是否有问题，如果仍然无法解决，请咨询[技术支持](https://applink.feishu.cn/TLJpeNdW)。</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>用户 ID 无效。你可以通过以下接口获取指定用户的用户 ID，然后使用正确的用户 ID 再次尝试调用接口。

- [搜索用户](/ssl:ttdoc/ukTMukTMukTM/uMTM4UjLzEDO14yMxgTN)
- [通过手机号或邮箱获取用户 ID](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/batch_get_id)</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>内部错误。请联系[技术支持](https://applink.feishu.cn/TLJpeNdW)并提供请求头 X-Request-Id。</md-td>
</md-tr>
