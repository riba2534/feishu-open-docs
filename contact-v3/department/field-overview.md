...
<md-thead>
<md-tr>
<md-th style="width:30%">部门 ID 类型</md-th>
...
<md-td>
该类型的 ID 在创建部门时支持自定义配置，若不自定义配置，系统会默认生成一个 ID 值。因此，如果你的企业内部系统已有维护中的部门 ID，你可以将这些部门 ID 写入到飞书通讯录的部门 ID 属性中，由此实现飞书部门 ID 和企业内部系统部门 ID 的一致性，节省跨系统调用的映射成本。
...
- 通过部门资源 API 获取。
	- 调用[获取子部门列表](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/children)接口，传入根部门的 ID（department_id 取值为 0），来获取根部门下的各个部门的 department_id。
	- 调用[搜索部门](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/search)接口，设置部门名称关键词，获取指定部门的 department_id。
</md-td>
</md-tr>
...
<md-td>
该类型的 ID 由系统自动生成，ID 前缀固定为 `od-`，无法自定义编辑。open_department_id 在租户内全局唯一。
  
**open_department_id 的获取方式：**

- 调用[获取子部门列表](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/children)接口，传入根部门的 ID（department_id 取值为 0），来获取根部门下的各个部门的 open_department_id。
- 调用[搜索部门](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/search)接口，设置部门名称关键词，获取指定部门的 open_department_id。
</md-td>
</md-tr>
...
<md-tr>
	...
	<md-td>
    支持用户自定义或者由系统默认生成的部门 ID。在用户自定义配置时可复用已删除的 department_id，因此在未删除的部门范围内 department_id 具有唯一性。
...
<md-tr>
	...
	<md-td>
	父部门的部门 ID。如果创建部门时，父部门是通讯录内的根部门，则该参数值为 `0`。
...
<md-tr>
	...
	<md-td>
	部门的排序，即部门在其同级部门内的展示顺序。取值格式为 String 类型的非负整数，数值越小，排序越靠前。
...
<md-tr>
	...
	<md-td>
	部门绑定的单位自定义 ID 列表，当前只支持绑定一个单位。了解单位信息参见[资源介绍](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/unit/overview)。
