...
| `parent_job_family_id` | `string` | 否 | 上级序列 ID。如果你需要为某一序列添加子序列，则需要传入该参数值。你可以调用[获取租户序列列表](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/job_family/list)接口，获取序列 ID。<br>默认值：空，表示该序列没有上级序列。<br>**示例值**："mga5oa8ayjlpzjq" |
| `status` | `boolean` | 是 | 是否启用序列。<br>**可选值有**：<br>- true：启用 - false：禁用<br>**示例值**：true |
