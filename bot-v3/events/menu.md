...
| 字段权限要求 | > **Tip**: 该接口返回体中存在下列敏感字段，仅当开启对应的权限后才会返回；如果无需获取这些字段，则不建议申请 `application:application.bot.operator_name:readonly` 获取机器人自定义菜单操作人名称 `contact:user.employee_id:readonly` 获取用户 user ID |
| 推送方式 | `Webhook` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `operator_name` | `string` | 用户名称<br>**数据校验规则**：<br>- 长度范围：`1` ～ `100` 字符<br>**字段权限要求**： `application:application.bot.operator_name:readonly` 获取机器人自定义菜单操作人名称 |
