---
title: "获取群成员列表"
fullPath: "/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-members/get"
updateTime: "1730257096000"
---
...
| `page_size` | `int` | 否 | 分页大小，用来限制一次请求所返回的数据条目数。<br>- 由于返回的群成员列表会过滤掉机器人成员，因此返回的群成员个数可能会小于指定的 page_size。  - 如果有同一时间加入群的群成员，会一次性返回，这会导致返回的群成员个数可能会大于指定的 page_size。<br>**示例值**：20<br>**默认值**：`20`<br>**数据校验规则**：<br>- 最大值：`100` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `member_id` | `string` | 成员的用户ID，ID 值与查询参数中的 member_id_type 对应。<br>不同 ID 的说明参见 [用户相关的 ID 概念](https://open.larkoffice.com/document/home/user-identity-introduction/introduction) |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `name` | `string` | 名字 |
