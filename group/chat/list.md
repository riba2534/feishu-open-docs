...
获取 [access_token](/ssl:ttdoc/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-choose-which-type-of-token-to-use) 所代表的用户或者机器人所在的群列表。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=im&version=v1&resource=chat&method=list)
...
**可选值有**：
<md-enum>
<md-enum-item key="ByCreateTimeAsc" >按群组创建时间升序排列</md-enum-item>
<md-enum-item key="ByActiveTimeDesc" >按群组活跃时间降序排列。因群组活跃时间变动频繁，使用 `ByActiveTimeDesc` 排序方式可能会造成群组遗漏。例如，设置分页大小为 10，发起第一次请求获取到第一页数据后，原本排在第 11 位的群组中有群成员发送了一条消息，那么该群组将被排列到第 1 位，此时发起请求获取第二页数据时，该群组将不能被获取到，需要再从第一页开始获取。</md-enum-item>
</md-enum>
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	<md-text type="field-type" >list_chat\[\]</md-text>
	</md-dt-td>
	<md-dt-td>
	群组列表
	</md-dt-td>
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	群组 ID，调用[获取群信息](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat/get)接口，根据群组 ID 可获取群详细信息。
	</md-dt-td>
</md-dt-tr>
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	群主的用户 ID，ID 类型与 owner_id_type 相对应。
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	群主的用户 ID 类型，类型分为 user_id、open_id、union_id。各类型 ID 介绍参考[用户身份概述](/ssl:ttdoc/home/user-identity-introduction/introduction)。
...
<md-dt-tr level="2">
	<md-dt-td>
	<md-text type="field-name" >chat_status</md-text>
	</md-dt-td>
	<md-dt-td>
	<md-text type="field-type" >string</md-text>
	</md-dt-td>
...
<md-enum-item key="dissolved" >解散</md-enum-item>
<md-enum-item key="dissolved_save" >解散并保留</md-enum-item>
</md-enum>
	</md-dt-td>
</md-dt-tr>
