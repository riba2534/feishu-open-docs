...
调整指定群组内的群菜单排列顺序，成功调用后接口会返回群组内所有群菜单信息。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=im&version=v1&resource=chat.menu_tree&method=sort)
...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.feishu.cn/open-apis/im/v1/chats/:chat_id/menu_tree/sort</md-td>
    </md-tr>
    <md-tr>
...
<md-dt-tr level="0">
	<md-dt-td>
	<md-text type="field-name" >chat_menu_top_level_ids</md-text>
	</md-dt-td>
	<md-dt-td>
	<md-text type="field-type" >string\[\]</md-text>
	</md-dt-td>
	...
	<md-dt-td>
	通过一级菜单的 ID 进行排序。数组内的元素排序对应群组内一级菜单从左往右的排序。ID 可通过 [获取群菜单](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-menu_tree/get) 接口获取。
...
**可选值有**：
<md-enum>
<md-enum-item key="NONE" >无分类，当一级菜单下有二级菜单时，类型取值为 NONE。</md-enum-item>
...
<md-dt-tr level="4">
	...
	<md-dt-td>
	二级菜单 ID，后续删除、修改、排序等群菜单管理操作均需要使用菜单 ID。
	</md-dt-td>
</md-dt-tr>
...
<md-tr>
  <md-td>400</md-td>
  <md-td>232072</md-td>
  <md-td>Sort chat_menu_top_level's param is invalid.</md-td>
  <md-td>传入的一级菜单 ID 列表跟群内目前存在的一级菜单 ID 列表不一致。请检查参数传值是否有误。</md-td>
</md-tr>
