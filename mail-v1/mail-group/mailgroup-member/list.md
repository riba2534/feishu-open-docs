...
| `department_id_type` | `string` | 否 | 此次调用中使用的部门ID的类型<br>**示例值**：open_department_id<br>**可选值有**：<br>- `department_id`: 以自定义department_id来标识部门 - `open_department_id`: 以open_department_id来标识部门 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `type` | `string` | 成员类型<br>**可选值有**：<br>- `USER`: 内部用户 - `DEPARTMENT`: 部门 - `COMPANY`: 全员 - `EXTERNAL_USER`: 外部用户 - `MAIL_GROUP`: 邮件组 - `PUBLIC_MAILBOX`: member is a public mailbox - `OTHER_MEMBER`: 内部成员 |
