...
> **Tip**: **说明**：飞书客户端在 V7.6 及以上版本开始支持应用消息流卡片。
...
| `user_ids` | `string\[\]` | 否 | 用户 ID 列表（ID 类型与 user_id_type 的取值一致。如果 是商店应用，因不支持获取用户 userID 权限，所以无法值使用 user_id 类型的用户 ID）<br>**示例值**：["ou_a0553eda9014c201e6969b478895c230"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `20` |
