...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `corehr:common_data.preset_data:read` 获取预置数据信息 `corehr:corehr:readonly` 获取核心人事信息 `corehr:transfer_reason:read` 获取异动原因信息 `corehr:common_data.preset_data:write` 更新预置数据信息 `corehr:corehr` 更新核心人事信息 |
