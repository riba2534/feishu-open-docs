---
title: "异动状态变更（不推荐）"
fullPath: "/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job_change/events/updated"
updateTime: "1734431977000"
---
...
| 支持的应用类型 | custom,isv |
| 权限要求             订阅该事件所需的权限，开启其中任意一项权限即可订阅 开启任一权限即可 | `corehr:corehr:readonly` 获取核心人事信息 `corehr:job_change.update_event:read` 获取员工异动状态变更信息 `corehr:job_change:read` 获取员工异动信息 `corehr:job_change:write` 读写员工异动信息 `corehr:job_data:readonly` 获取任职信息 |
...
| `event` | `\-` | \- |
| &nbsp;&nbsp;└ `employment_id` | `string` | 雇员ID，可通过[【搜索员工信息】](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/employee/search)接口获取 |
| &nbsp;&nbsp;└ `target_user_id` | `user_id` | 用户 ID，飞书相关ID， 可通过[【通讯录接口】](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/field-overview)接口获取 |
...
| &nbsp;&nbsp;└ `transfer_mode` | `int` | 异动属性/方式<br>**可选值有**：<br>- `1`: 直接异动：无需审批的异动，操作后异动状态为「无需审批」 - `2`: 发起异动：需要走异动流程 |
| &nbsp;&nbsp;└ `transfer_type_unique_identifier` | `string` | 异动类型唯一标识，可通过接口 [获取异动类型列表 ](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/transfer_type/query)获取 |
...
| &nbsp;&nbsp;└ `effective_date` | `string` | 异动生效日期，格式："YYYY-MM-DD" - 示例：2022-04-03 |
| &nbsp;&nbsp;└ `status` | `int` | 异动状态<br>**可选值有**：<br>- `1`: 异动发起 = 审批中状态 - `2`: 已撤销 - `3`: 已拒绝 - `4`: 审批通过 - `5`: 已生效（到了生效日期后就生效） - `6`: 无需审批 |
