...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `corehr:cost_allocation:write` 成本分摊写权限 |
...
| &nbsp;&nbsp;└ `wk_id` | `string` | 是 | wk_id -可以调用[【查询成本分摊】](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/cost_allocation/batch_query)接口，获取对应成本分摊信息的成本分摊ID。<br>**示例值**："703912325303191204" |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `cost_center_id` | `string` | 否 | 成本中心 ID -可以调用[【搜索成本中心信息】](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/cost_center/search)接口，获取对应成本中心信息的成本中心ID。<br>**示例值**："6950635856373745165" |
