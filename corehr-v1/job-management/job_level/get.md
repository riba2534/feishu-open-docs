...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `corehr:corehr:readonly` 获取核心人事信息 `corehr:job_level:read` 获取职级信息 `corehr:corehr` 更新核心人事信息 `corehr:job_level:write` 读写职级信息 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| `job_level_id` | `string` | 职级ID。ID获取方式： - 调用[【新建职级】](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job_level/create)[【查询租户的职级信息】](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job_level/list)等接口可以返回职级ID<br>**示例值**："1515" |
