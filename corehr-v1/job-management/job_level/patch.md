...
该接口通过职级ID更新单个职级信息，包括职级数值、名称等信息。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=corehr&version=v1&resource=job_level&method=patch)
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	职级数值，单位：级。该字段主要用来在职级大小，职级的数值越大，代表职级越高
- 如果不填该字段则不更新
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	职级编码。非必填字段，如果非空值会校验全局唯一性，如果传空值则不参与全局校验。
- 不传值时默认不更新此字段，保持原值
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	字段值，是json转义后的字符串，具体传值方式参见[获取自定义字段的元数据](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/custom-fields-guide)
- 最小1字符，最大200字符
