---
title: "批量查询序列"
fullPath: "/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job_family/list"
...
> **Warning**: 延迟说明：数据库主从延迟 2s 以内，即：直接创建序列后2s内调用此接口可能查询不到数据。


> **Tip**: 使用建议：序列数量过多时，可以通过多次循环调用该接口获取所有序列详情信息，包括序列编码、名称等。
...
| `page_size` | `string` | 是 | 分页大小，最大值支持传100 - 最少1个字符，最多200个字符<br>**示例值**：100 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `effective_time` | `string` | 当前版本生效时间 - 返回格式：YYYY-MM-DD 00:00:00（最小单位到日） - 日期范围:1900-01-01 00:00:00～9999-12-31 00:00:00 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `expiration_time` | `string` | 当前版本失效时间 - 返回格式：YYYY-MM-DD 00:00:00（最小单位到日） - 日期范围:1900-01-01 00:00:00～9999-12-31 00:00:00 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `code` | `string` | 序列编码 |
| &nbsp;&nbsp;&nbsp;&nbsp;└ `description` | `i18n\[\]` | 描述 |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `lang` | `string` | 语言 |
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `value` | `string` | 内容 |
