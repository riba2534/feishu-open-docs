...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `corehr:authorization:read` 获取用户授权数据 `corehr:corehr:readonly` 获取核心人事信息 `corehr:corehr` 更新核心人事信息 `corehr:authorization.bp:read` 查询部门/地点的HRBP/属地BP |
