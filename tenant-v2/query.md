...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `tenant:tenant:readonly` 获取企业信息 |
| 字段权限要求 | > **Tip**: 该接口返回体中存在下列敏感字段，仅当开启对应的权限后才会返回；如果无需获取这些字段，则不建议申请 `tenant:tenant.domain:read` 获取企业完整域名 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `tenant_tag` | `int` | 个人版/团队版标志<br>**可选值有**：<br>- `0`: 团队版 - `2`: 个人版 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `domain` | `string` | 企业完整域名。企业域名可用于企业成员访问管理后台、云文档等含URL地址的网页。<br>**字段权限要求**： `tenant:tenant.domain:read` 获取企业完整域名 |
