...
| &nbsp;&nbsp;└ `type` | `int` | 否 | 记录的生成方式。举例：type=0表示「开放平台导入」的「用户打卡」流水；type=1表示「开放平台导入」的「管理员修改」流水。若不设置type，则默认是0。<br>**示例值**：7<br>**可选值有**：<br>- `0`: 用户打卡 - `1`: 管理员修改 - `2`: 用户补卡 - `3`: 系统自动生成 - `4`: 下班免打卡 - `5`: 考勤机 - `6`: 极速打卡 - `7`: 考勤开放平台导入 |
...
| &nbsp;&nbsp;└ `check_result` | `string` | 否 | 打卡结果，作为入参时无效<br>**示例值**："Invalid"<br>**可选值有**：<br>- `NoNeedCheck`: 无需打卡 - `SystemCheck`: 系统打卡 - `Normal`: 正常 - `Early`: 早退 - `Late`: 迟到 - `SeriousLate`: 严重迟到 - `Lack`: 缺卡 - `Invalid`: 无效 - `None`: 无状态 - `Todo`: 尚未打卡 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `check_result` | `string` | 打卡结果<br>**可选值有**：<br>- `NoNeedCheck`: 无需打卡 - `SystemCheck`: 系统打卡 - `Normal`: 正常 - `Early`: 早退 - `Late`: 迟到 - `SeriousLate`: 严重迟到 - `Lack`: 缺卡 - `Invalid`: 无效 - `None`: 无状态 - `Todo`: 尚未打卡 |
