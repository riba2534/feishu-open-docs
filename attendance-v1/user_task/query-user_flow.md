...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `check_result` | `string` | 无效字段。如需获取打卡结果请使用[获取打卡结果](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/attendance-v1/user_task/query)<br>**可选值有**：<br>- `NoNeedCheck`: 无需打卡 - `SystemCheck`: 系统打卡 - `Normal`: 正常 - `Early`: 早退 - `Late`: 迟到 - `SeriousLate`: 严重迟到 - `Lack`: 缺卡 - `Invalid`: 无效 - `None`: 无状态 - `Todo`: 尚未打卡 |
