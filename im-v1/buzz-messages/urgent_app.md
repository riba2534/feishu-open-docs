...
- 不支持加急[批量发送的消息](https://open.larkoffice.com/document/ukTMukTMukTM/ucDO1EjL3gTNx4yN4UTM)。
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `im:message.urgent` 发送应用内加急消息 `im:message.urgent:app_send` 发送应用内加急消息（历史版本） |
