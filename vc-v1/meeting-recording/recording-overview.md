...
updateTime: "1679916459000"
---

# 资源介绍
##  资源定义
用户可以录制一场会议，在会议结束后获得会议录制文件链接，包括：[开始录制](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/meeting-recording/start)、[停止录制](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/meeting-recording/stop)、[获取录制文件](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/meeting-recording/get)、[授权录制文件](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/meeting-recording/set_permission)。
...
| `timezone` | `int` | 录制文件时间显示使用的时区[-12,12] **示例值**：8 |
| `data` | `-` | `-` |
...
| ∟ `type` | `int` | 授权对象类型            **示例值**：1            **可选值有**： - `1`：用户授权 - `2`：群组授权 - `3`：租户内授权（id字段不填） - `4`：公网授权（id字段不填） |
| ∟ `permission` | `int` | 权限                       **示例值**：1                       **可选值有**： - `1`：查看 |
