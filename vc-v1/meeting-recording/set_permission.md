...
> **Warning**: 会议结束后并且收到了"录制完成"的事件方可进行授权；会议owner（通过开放平台预约的会议即为预约人）才有权限操作
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `vc:record` 更新会议录制信息 |
...
| &nbsp;&nbsp;└ `type` | `int` | 是 | 授权对象类型<br>**示例值**：1<br>**可选值有**：<br>- `1`: 用户授权（id字段填入用户ID） - `2`: 群组授权（id字段填入群组open_chat_id） - `3`: 租户内授权（id字段不填） - `4`: 公网授权（id字段不填） |
| &nbsp;&nbsp;└ `permission` | `int` | 是 | 权限<br>**示例值**：1<br>**可选值有**：<br>- `1`: 查看 |
| `action_type` | `int` | 否 | 授权或者取消授权，默认授权<br>**示例值**：1<br>**可选值有**：<br>- `0`: 授权 - `1`: 取消授权 |
