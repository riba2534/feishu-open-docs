...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `document_ai:chinese_passport:recognize` 识别中国护照 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `type` | `string` | 识别的字段种类<br>**可选值有**：<br>- `full_name_cn`: 中文姓名 - `full_name_en`: 英文格式姓名 - `date_of_birth`: 出生日期 - `date_of_expiry`: 有效期至 - `place_of_issue`: 签发地点 - `passport_number`: 护照号码 |
