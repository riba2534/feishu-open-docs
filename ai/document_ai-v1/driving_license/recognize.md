...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `document_ai:driving_license:recognize` 驾驶证识别 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `type` | `string` | 识别的字段种类<br>**可选值有**：<br>- `id_number`: 证号 - `name`: 姓名 - `sex`: 性别 - `nationality`: 国籍 - `address`: 住址 - `date_of_birth`: 出生日期 - `date_of_first_issue`: 初次领证日期 - `class`: 准驾车型 - `valid_begin`: 有效期起 - `valid_end`: 有效期止 - `license_issuing_authority`: 发证机关 - `document_id`: 档案编号 - `record`: 记录 - `id_photo_location`: 相片位置 |
