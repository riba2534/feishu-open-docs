...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `document_ai:business_card:recognize` 识别名片 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `type` | `string` | 识别的字段种类<br>**可选值有**：<br>- `contact_names`: 联系人名 - `company_names`: 公司名 - `departments`: 部门 - `job_titles`: 职位 - `emails`: 邮箱 - `websites`: 网站 - `addresses`: 地址 - `mobile_phones`: 移动电话 - `work_phones`: 工作电话 - `other_phones`: 其他电话 - `faxes`: 传真 |
