...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `document_ai:taxi_invoice:recognize` 出租车发票识别 |
...
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ `type` | `string` | 识别的字段种类<br>**可选值有**：<br>- `car_number`: 车号 - `start_time`: 上车时间 - `end_time`: 下车时间 - `distance`: 里程 - `start_date`: 日期 - `total_amount`: 出租车价格 - `invoice_code`: 发票代码 - `invoice_no`: 发票号码 - `price`: 单价 - `dispatch_fee`: 电召费 - `additional_fee`: 附加费 - `is_sealed`: 是否盖章 - `seller_name_in_seal`: 印章内销售方名称 - `seller_taxpayer_no_in_seal`: 印章内销售方税号 - `title_trial`: 发票名称 - `invoice_special_seal`: 印章名称 |
