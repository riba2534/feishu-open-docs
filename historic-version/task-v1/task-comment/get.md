---
title: "获取评论详情"
fullPath: "/uAjLw4CM/ukTMukTMukTM/reference/task-v1/task-comment/get"
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `task:comment` 查看、创建、修改、删除任务评论（历史版本） `task:comment:readonly` 查看任务评论（历史版本） |
