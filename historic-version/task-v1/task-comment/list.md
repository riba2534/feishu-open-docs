...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `task:task` 查看、创建、编辑和删除飞书任务（历史版本） `task:task:readonly` 查看任务详情 |
...
| `list_direction` | `int` | 否 | 评论排序标记，可按照评论时间从小到大查询，或者评论时间从大到小查询，不填默认按照从小到大<br>**示例值**：0<br>**可选值有**：<br>- `0`: 按照回复时间从小到大查询 - `1`: 按照回复时间从大到小查询<br>**默认值**：`0` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;└ `rich_content` | `string` | 富文本评论内容。语法格式参见[Markdown模块](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/task-v1/markdown-module) |
