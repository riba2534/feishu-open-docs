...
该接口用于获取任务详情，包括任务标题、描述、时间、成员等信息。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=task&version=v2&resource=task&method=get)

:::html
<md-alert type="tip">
获取任务详情需要调用身份拥有对任务的可读取权限。详情见[任务功能概述](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“任务是如何鉴权的？”章节。
</md-alert>
:::
...
<md-table>
  ...
  <md-tbody>
    ...
    <md-tr>
      ...
      <md-td>
            <md-perm name="task:task:read" desc="查看任务信息" support_app_types="custom,isv" tags="">查看任务信息</md-perm>
...
<md-dt-tr level="3">
	...
	<md-dt-td>
	截止时间/日期的时间戳，距1970-01-01 00:00:00的毫秒数。
...
<md-tr>
  <md-td>400</md-td>
  <md-td>1470400</md-td>
  <md-td>请求参数错误，例如传入的任务GUID不合法。</md-td>
...
<md-tr>
  <md-td>403</md-td>
  <md-td>1470403</md-td>
  <md-td>缺少对任务的访问权限。</md-td>
  <md-td>检查调用身份是否有权限访问任务数据。详情见[任务功能概述](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“任务是如何鉴权的？”章节。</md-td>
</md-tr>
