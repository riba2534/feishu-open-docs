...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `task:tasklist:write` 查看、创建、更新、删除协作清单 |
...
| `update_fields` | `string\[\]` | 否 | 要更新的字段名，支持 * `name`:  选项名称 * `color_index`: 选项的颜色索引值 * `is_hidden`: 是否从界面上隐藏 * `insert_before`: 将当前option放到同字段某个option之前。 * `insert_after`: 将当前option放到同字段某个option之后。<br>**示例值**：["name"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `20` |
