...
| HTTP Method | GET |
| 支持的应用类型 | custom |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 | `acs:access_record:readonly` 查看智能门禁记录 |
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `page_size` | `int` | 否 | 分页大小<br>**示例值**：100<br>**默认值**：`100`<br>**数据校验规则**： - 最大值：`500` |
...
| `to` | `int` | 是 | 记录结束时间，单位秒， 时间跨度不能超过30天<br>**示例值**：1624520521 |
