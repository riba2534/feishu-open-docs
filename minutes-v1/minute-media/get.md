...
> **Tip**: 通过接口获取妙记音视频文件的下载链接（有效期1天），以进行批量下载
...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `minutes:minute:download` 导出妙记文件 `minutes:minutes.media:export` 下载妙记的音视频文件 > **Tip**: “导出妙记文件”权限不再开放，已添加权限的应用不受影响，新应用请使用“下载妙记的音视频文件”权限。 |
