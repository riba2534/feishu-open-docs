...
> **Tip**: - 如果使用应用身份调用该接口，则需要确保应用开启了[机器人能力](https://open.larkoffice.com/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-enable-bot-ability)。
> - 当前身份必须对日历有访问权限。
...
| 名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| `calendar_ids` | `string\[\]` | 是 | 日历 ID 列表，多个 ID 传入格式为 `["xxx1", "xxx2"]`。<br>创建共享日历时会返回日历 ID。你也可以调用以下接口获取某一日历的 ID。 - [查询主日历信息](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/primary) - [查询日历列表](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/list) - [搜索日历](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/search)<br>**示例值**：["feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `10` |
