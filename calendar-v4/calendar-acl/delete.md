...
| 支持的应用类型 | custom,isv |
| 权限要求             调用该 API 所需的权限。开启其中任意一项权限即可调用 开启任一权限即可 | `calendar:calendar` 更新日历及日程信息 `calendar:calendar.acl:delete` 删除日历访问控制权限 |
...
| `acl_id` | `string` | 访问控制 ID。<br>为日历创建访问控制时会返回访问控制 ID。你也可以调用[获取访问控制列表](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-acl/list)接口，获取指定日历内的访问控制信息。<br>**示例值**："user_xxxxxx" |
